...
public class Pessoa extends HttpServlet {
    ...
    public String lervarios() {
        JsonResult<List<contratos.Pessoa>> json = new JsonResult<List<contratos.Pessoa>>();

        database.Pessoa data = new database.Pessoa();
        List<contratos.Pessoa> pessoas = data.listar();

        if (pessoas == null) {
            json.mensagem = "Houve um erro ao carregar as pessoas.";
            json.sucesso = false;
        } else {
            json.resultado = pessoas;
            json.sucesso = true;
        }
