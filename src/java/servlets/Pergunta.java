...
public class Pergunta extends HttpServlet {
    ...
    public String lervarios() {
        JsonResult<List<contratos.Pergunta>> json = new JsonResult<List<contratos.Pergunta>>();

        database.Pergunta data = new database.Pergunta();
        List<contratos.Pergunta> perguntas = data.listar();

        if (perguntas == null) {
            json.mensagem = "Houve um erro ao carregar as perguntas.";
            json.sucesso = false;
        } else {
            json.resultado = perguntas;
            json.sucesso = true;
        }
