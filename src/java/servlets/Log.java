...
public class Log extends HttpServlet {
    ...
    public String lervarios(String json) {
        JsonResult<List<contratos.Log>> result = new JsonResult<List<contratos.Log>>();

        try {
            JsonReceiver<contratos.LogFilter> receiver = new JsonReceiver<>(contratos.LogFilter.class);
            receiver.Desserealizar(json);
            contratos.LogFilter filter = receiver.getData();
            List<contratos.Log> logs = new database.Log().getList(filter);
            result.sucesso = true;
            result.resultado = logs;
        } catch (Exception e) {
            library.Log.writeError("Erro ao carregar os logs por filtro.", e.toString(), ModuloEnum.LOG, json);
