 ...
 * @author Bruno
 */
public class ContaBancaria extends HttpServlet {
    ...
    public String lervarios() {
        JsonResult<List<contratos.ContaBancaria>> json = new JsonResult<List<contratos.ContaBancaria>>();

        database.ContaBancaria data = new database.ContaBancaria();
        List<contratos.ContaBancaria> contasBancarias = data.listar();

        if (contasBancarias == null) {
            json.mensagem = "Houve um erro ao carregar as contas bancárias.";
            json.sucesso = false;
        } else {
            json.resultado = contasBancarias;
            json.sucesso = true;
        }
    ...
    private String Editar(String json) {
        contratos.JsonReceiver<contratos.ContaBancaria> c = new JsonReceiver<>(contratos.ContaBancaria.class);
        c.Desserealizar(json);

        contratos.ContaBancaria conta = c.getData();
        Boolean resultado = new database.ContaBancaria().editar(conta);
        ...
        if (resultado) {
            ...
            result.mensagem = "Não foi possível editar a conta bancária.";
    ...
    private String Inserir(String json) {
        JsonReceiver<contratos.ContaBancaria> recebido = new JsonReceiver<>(contratos.ContaBancaria.class);
        recebido.Desserealizar(json);

        database.ContaBancaria c = new database.ContaBancaria();
        ...
        if (resultado) {
            ...
            result.resultado = false;
            result.sucesso = false;
            result.mensagem = "Não foi possível salvar a conta bancária.";
        }
    ...
    private String remover(String receiveJson) {
        ...
        if (resultado) {
            ...
            json.mensagem = "Não foi possível excluir a conta bancária.";
        }
