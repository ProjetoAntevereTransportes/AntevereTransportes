 ...
 */
public class StatusUsuario extends HttpServlet {
    ...
    public String lervarios() {
        JsonResult<List<contratos.StatusUsuario>> json = new JsonResult<List<contratos.StatusUsuario>>();

        database.StatusUsuario data = new database.StatusUsuario();
        List<contratos.StatusUsuario> lstatus = data.listar();

        if (lstatus == null) {
            json.mensagem = "Houve um erro ao carregar os status.";
            json.sucesso = false;
        } else {
            json.resultado = lstatus;
            json.sucesso = true;
        }
